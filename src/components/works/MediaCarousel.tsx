...
export default function MediaCarousel({
    ...
    const next = () => setI((v) => (v + 1) % uniqSlides.length);
    const prev = () => setI((v) => (v - 1 + uniqSlides.length) % uniqSlides.length);
    const swipe = useSwipeable({
        onSwipedLeft: next,
        onSwipedRight: prev,
        preventScrollOnSwipe: true,
        trackTouch: true,
        // set to true if you also want mouse-drag on desktop:
        trackMouse: true,
        delta: 10, // swipe sensitivity (px)
    });
